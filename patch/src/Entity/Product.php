...
class Product
{
    ...
    // NOUVEAUX CHAMPS: Prix d'achat et marge
    #[ORM\Column(type: Types::DECIMAL, precision: 15, scale: 2, nullable: true)]
    #[Assert\PositiveOrZero(message: 'Le prix d\'achat doit être positif ou nul.')]
    private ?string $purchasePrice = null;

    #[ORM\Column(type: Types::DECIMAL, precision: 5, scale: 2, nullable: true)]
    #[Assert\PositiveOrZero(message: 'La marge doit être positive ou nulle.')]
    ...
    public function getPurchasePriceFloat(): float
    {
        return (float) ($this->purchasePrice ?? 0);
    }
    ...
    public function getMarginFloat(): float
    {
        return (float) ($this->margin ?? 0);
    }
    ...
    /**
     * Calcule le prix de vente suggéré basé sur le prix d'achat et la marge
     */
    public function calculateSuggestedPrice(?float $defaultMargin = null): float
    {
        $purchasePrice = $this->getPurchasePriceFloat();
        if ($purchasePrice <= 0) {
            return $this->getUnitPriceFloat();
        }

        $margin = $this->getMarginFloat();
        if ($margin <= 0 && $defaultMargin !== null) {
    ...
    public function getFormattedCharacteristics(): string
    {
        ...
        switch ($this->type) {
            case self::TYPE_LOGICIEL:
                if ($this->version) $parts[] = "Version: {$this->version}";
                if ($this->licenseType) $parts[] = "Licence: {$this->licenseType}";
                if ($this->maxUsers) $parts[] = "{$this->maxUsers} utilisateur(s)";
                if ($this->licenseDuration) $parts[] = "Durée: {$this->licenseDuration} mois";
                break;

            case self::TYPE_MATERIEL:
                if ($this->brand) $parts[] = $this->brand;
                if ($this->model) $parts[] = $this->model;
                if ($this->warrantyMonths) $parts[] = "Garantie: {$this->warrantyMonths} mois";
                break;

            case self::TYPE_SERVICE:
                if ($this->durationHours) $parts[] = "Durée: {$this->durationHours}h";
                break;
        }
