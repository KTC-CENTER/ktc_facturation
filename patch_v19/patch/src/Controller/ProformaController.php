...
class ProformaController extends AbstractController
{
    ...
    public function new(Request $request): Response
    {
        ...
        $proforma->setTaxRate((string) ($settings?->getDefaultTaxRate() ?? 19.25));
        $proforma->setConditions($settings?->getDefaultProformaConditions());
        ...
        // Pré-remplir avec un produit si spécifié (depuis fiche produit)
        $productId = $request->query->get('product_id');
        if ($productId && $proforma->getItems()->isEmpty()) {
            $product = $this->productRepository->find($productId);
            if ($product) {
                $item = new DocumentItem();
                $item->setProduct($product);
                $item->setDesignation($product->getName());
    ...
    #[IsGranted('ROLE_COMMERCIAL')]
    public function sendEmail(Request $request, Proforma $proforma, \App\Service\BrevoMailerService $mailer): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if ($success) {
                ...
                if ($proforma->getStatus() === Proforma::STATUS_DRAFT) {
                    $proforma->setStatus(Proforma::STATUS_SENT);
                    $this->entityManager->flush();
                }
    ...
    #[IsGranted('ROLE_COMMERCIAL')]
    public function sendWhatsApp(Proforma $proforma): Response
    {
        $client = $proforma->getClient();
        $settings = $this->settingsRepository->getSettings();
        ...
        $phone = preg_replace('/[^0-9]/', '', $client->getPhone());
        ...
        $message = sprintf(
            "Bonjour %s,\n\nVoici votre proforma *%s* d'un montant de *%s %s*.\n\nCordialement,\n%s",
            $client->getName(),
            $proforma->getReference(),
            number_format($proforma->getTotalTTCFloat(), 0, ',', ' '),
            $currency,
            $companyName
        );
        ...
        // Redirect directly to WhatsApp (opens in new tab from template with target="_blank")
